...
#define currBB_conf_threshold	0.5

using namespace std;
using namespace cv;

void* trackingThread(void* lpParam) {
	tld::TLD* tld = (tld::TLD*) lpParam;

	tld->medianFlowTracker->track(tld->prevImg, tld->currImg, tld->prevBB);

	pthread_exit(0);
}

void* detectionThread(void* lpParam) {
	detectInfo* info = (detectInfo*) lpParam;

	info->tld->detectorCascade->detect(info->grey_frame);
...
void TLD::processImage(const Mat &img) {
	...
	pthread_t idTrackingThread;
	pthread_t idDetectionThread;

	if (trackerEnabled) {
		pthread_create(&idTrackingThread, NULL, trackingThread, this);
	...
	if (detectorEnabled
		...
		info->tld = this;
		info->grey_frame = grey_frame;
		pthread_create(&idDetectionThread, NULL, detectionThread, info);
		//detectorCascade->detect(grey_frame);
	}

	pthread_join(idTrackingThread, NULL);
	pthread_join(idDetectionThread, NULL);
...
void TLD::fuseHypotheses() {
	...
	if (currConf < currBB_conf_threshold) {
		if (currBB) {
			memcpy(finalBB, currBB, sizeof(Rect));
			delete currBB;
			currBB = NULL;
		}
	}

	if (currBB == NULL && bCallback) {
		if (m_cbObjLeaved) {
			int32_t sizeOfTruePositives = nnClassifier->truePositives->size();
			int32_t sizeOfFalsePositives = nnClassifier->falsePositives->size();
			int32_t eachElementSize = TLD_PATCH_SIZE * TLD_PATCH_SIZE;
			uint32_t totFeatureSize = (sizeOfTruePositives + sizeOfFalsePositives) * eachElementSize;
			float* feature = new float[totFeatureSize];
			uint32_t featureSizeInByte = sizeof(float) * totFeatureSize;
			int32_t indexCounter = 0;
			for (int32_t i = 0; i < sizeOfTruePositives; ++i)
				for (int32_t j = 0; j < eachElementSize; ++j)
					feature[indexCounter++] = nnClassifier->truePositives->at(i).values[j];
			int32_t splitIndex = indexCounter;
			for (int32_t i = 0; i < sizeOfFalsePositives; ++i)
				for (int32_t j = 0; j < eachElementSize; ++j)
					feature[indexCounter++] = nnClassifier->falsePositives->at(i).values[j];

			m_cbObjLeaved(m_userData, m_objId, feature, featureSizeInByte,
					splitIndex, totFeatureSize, eachElementSize);
			delete [] feature;
		}
		bCallback = false;
	}
...
void TLD::initialLearning() {
	...
	for (size_t i = 0; i < std::min < size_t > (100, negativeIndices.size());
			i++) {
...
void TLD::setCallbackFunc(string& objId, CbFunc Callback, void* userData) {
	m_objId = objId;
	m_userData = userData;
	m_cbObjLeaved = Callback;
}
